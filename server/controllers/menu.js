...
export const getMenuItems = async (req, res, next) => {
    try {
        const menuItems = await menuModel.find({ user_id: req.user[0]._id });
...
export const getOneItem = async (req, res, next) => {
    ...
    try {
        const item = await menuModel.find({ _id: id, user_id: req.user[0]._id });
...
export const addMenuItems = async (req, res, next) => {
    const item = req.body;
    const newItem = new menuModel({ ...item, user_id: req.user[0]._id });
    try {
        await newItem.save();
...
export const updateMenuItems = async (req, res, next) => {
    ...
    try {
        ...
        const updates = await menuModel.findByIdAndUpdate(_id, {...updatedData, user_id: req.user[0]._id}, {
            new: true,
        });
